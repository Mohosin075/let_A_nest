...
const createHostterms = async (
  ...
  try {
    ...
    if (terms) {
      ...
      terms = await Hostterms.findOneAndUpdate(filter, {...payload , isDefault : true}, { new: true, runValidators: true });
      return terms!;
    }

    // If not exist, create new terms
    const newTerms = await Hostterms.create({ ...payload, isDefault : true, hostId: user.authId });
    return newTerms;
...
const getAllHosttermss = async (
  ...
  const [result, total] = await Promise.all([
    Hostterms
      ...
      .skip(skip)
      .limit(limit)
      .sort({ [sortBy]: sortOrder }).populate('hostId').populate('propertyId'),
...
const getHostDefaultTerms = async(user : JwtPayload)=>{
  const result = await Hostterms.findOne({hostId : user.authId, isDefault : true});
  return result
}

const getSingleHostterms = async (id: string, user : JwtPayload): Promise<IHostterms> => {
  if (!Types.ObjectId.isValid(id)) {
    throw new ApiError(StatusCodes.BAD_REQUEST, 'Invalid Hostterms ID');
  }

  const result = await Hostterms.findOne({propertyId : id, hostId : user.authId})
  if (!result) {
    throw new ApiError(
...
export const updateHostterms = async (
  ...
  try {
    ...
    if (terms) {
      // Update and return the new document
      terms = await Hostterms.findOneAndUpdate(filter, {...payload, propertyId}, { new: true, runValidators: true });
      return terms!;
    }

    // If not exist, create new terms
    const newTerms = await Hostterms.create({ ...payload, hostId: user.authId, propertyId });
    return newTerms;
