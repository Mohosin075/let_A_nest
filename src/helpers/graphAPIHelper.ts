...
export async function publishInstagramMedia(
  ...
  const res = await fetch(`https://graph.facebook.com/v23.0/${igBusinessId}/media_publish`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
// not completed
export async function getInstagramPostInsights(igMediaId: string, pageAccessToken: string) {
  const res = await fetch(
