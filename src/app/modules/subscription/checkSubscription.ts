...
export const checkAndIncrementUsage = async (
  ...
  const plan = await Plan.findOne({ price: 0, status: 'active' }).session(session);
  ...
  const usageMap: Record<any, keyof typeof subscription.usage> = {
    reels: 'reelsUsed',
    post: 'postsUsed',
    story: 'storiesUsed',
  ...
  const limitMap: Record<any, keyof IPlan['limits']> = {
    reels: 'reelsPerWeek',
    post: 'postsPerWeek',
  ...
  await Subscription.findByIdAndUpdate(subscription._id, { $inc: { [usageKey]: 1 } }, { session });
